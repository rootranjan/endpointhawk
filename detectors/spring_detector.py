...
class SpringBootDetector(BaseDetector):
    ...
    def _extract_spring_variables(self, content: str) -> Dict[str, str]:
        """Dynamic runtime variable extraction with comprehensive Spring template literal support"""
        variables = {}
        
        # Step 1: Extract all variable declarations dynamically
        var_patterns = [
            # Final String constants
            r'final\s+String\s+(\w+)\s*=\s*[\'"`]([^\'"`,]+)[\'"`]',
        ...
        # Step 2: Extract Spring property placeholders
        property_patterns = [
            r'@Value\s*\(\s*[\'"`]\$\{([^}]+)\}[\'"`]\s*\)',
            r'@Value\s*\(\s*[\'"`]([^\'"`,]+)[\'"`]\s*\)',
        ]
            ...
            for match in matches:
                if isinstance(match, str):
                    if match.startswith('${') and match.endswith('}'):
                        ...
                        # Direct value
                        var_name = match.replace('.', '_').replace('-', '_').upper()
                        variables[var_name] = match
        
        # Step 3: Extract common Spring property placeholders
        ...
        for pattern in import_patterns:
            ...
            for import_statement in matches:
                ...
                variables[class_name] = f"IMPORT:{import_statement}"
        
        # Step 5: Dynamic template literal resolution
        # Find all Spring property placeholders and resolve them recursively
        template_patterns = [
            r'\$\{([^}]+)\}',  # Spring property placeholders
