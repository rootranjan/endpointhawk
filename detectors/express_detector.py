...
class ExpressDetector(BaseDetector):
    ...
    def __init__(self, framework: Framework = Framework.EXPRESS):
        ...
        # Comprehensive route patterns with enhanced coverage
        self.route_patterns = [
            ...
            # Template literal patterns with enhanced support
            re.compile(r'(router|app)\.(get|post|put|delete|patch|head|options|all)\s*\(\s*`([^`]*)`'),
            re.compile(r'(router|app)\.(use)\s*\(\s*`([^`]*)`'),
            ...
            # Enhanced route registration patterns
            re.compile(r'\.route\s*\(\s*[\'"`]([^\'"`,]+)[\'"`]\s*\)'),
            re.compile(r'\.route\s*\(\s*`([^`]*)`\s*\)'),
            
            # Dynamic route construction
            re.compile(r'(router|app)\.(get|post|put|delete|patch|head|options|all)\s*\(\s*([^,]+)\s*\+\s*[\'"`]([^\'"`,]+)[\'"`]'),
            re.compile(r'(router|app)\.(get|post|put|delete|patch|head|options|all)\s*\(\s*[\'"`]([^\'"`,]+)[\'"`]\s*\+\s*([^,]+)'),
            ...
            # Express Gateway specific patterns
            re.compile(r'pipeline\.(get|post|put|delete|patch|head|options|all)\s*\(\s*[\'"`]([^\'"`,]+)[\'"`]'),
            re.compile(r'pipeline\.(use)\s*\(\s*[\'"`]([^\'"`,]+)[\'"`]'),
            
            # Microservice patterns
            re.compile(r'service\.(get|post|put|delete|patch|head|options|all)\s*\(\s*[\'"`]([^\'"`,]+)[\'"`]'),
            re.compile(r'endpoint\.(get|post|put|delete|patch|head|options|all)\s*\(\s*[\'"`]([^\'"`,]+)[\'"`]'),
            
            # Proxy table patterns
    ...
    def detect_routes(self, file_path: str, content: str) -> List[RouteInfo]:
        ...
        # CRITICAL FIX: Filter out comments to prevent false positives like auth.service.js
        content = self._filter_comments_and_documentation(content)
            ...
            for pattern in self.route_patterns:
                for match in pattern.finditer(content):
                    ...
                    if len(groups) >= 3:
                        ...
                        # Create deduplication key - include line number to allow legitimate duplicates
                        route_key = (http_method.value, resolved_path, file_path, line_number)
    ...
    def _extract_variables(self, content: str) -> Dict[str, str]:
        """Dynamic runtime variable extraction with comprehensive template literal support"""
        variables = {}
        ...
        # Step 2: Extract object destructuring
        ...
        # Step 3: Extract import destructuring
    ...
    def _resolve_template_literal(self, raw_path: str, variables: Dict[str, str]) -> Tuple[str, List[str], List[str]]:
        """Dynamic runtime template literal resolution with comprehensive support"""
        resolved_path = raw_path
        path_params = []
        query_params = []
        
        # Step 1: Handle template literal expressions: ${variable}
        ...
        # Step 3: Extract Express-style parameters (:param)
        express_params = re.findall(r':(\w+)', resolved_path)
        path_params.extend(express_params)
        
        # Step 4: Remove duplicates from path_params
    ...
    def can_handle_file(self, file_path: str, content: str) -> bool:
        """Enhanced file filtering with intelligent exclusion patterns"""
        # Check file extension
        if not (file_path.endswith('.js') or file_path.endswith('.ts') or 
                file_path.endswith('.mjs') or file_path.endswith('.cjs')):
            return False
        
        # Enhanced exclusion patterns - filter out non-route files
        exclude_patterns = [
            # Test files
            ...
            r'babel\.config\.(js|ts)$',
            r'rollup\.config\.(js|ts)$',
            r'vite\.config\.(js|ts)$',
            r'jest\.config\.(js|ts)$',
            r'\.eslintrc\.(js|ts|json)$',
            r'\.prettierrc\.(js|ts|json)$',
            r'tsconfig\.json$',
            r'package\.json$',
            r'package-lock\.json$',
            r'yarn\.lock$',
        ...
        # Enhanced inclusion patterns - focus on route files
        include_patterns = [
            # Route directories
            r'routes?/',
            r'api/',
            ...
            r'\.route\.(js|ts|jsx|tsx)$',
            r'\.routes\.(js|ts|jsx|tsx)$',
            r'\.api\.(js|ts|jsx|tsx)$',
            r'\.controller\.(js|ts|jsx|tsx)$',
            
            # Main application files
            r'app\.(js|ts|jsx|tsx)$',
            r'server\.(js|ts|jsx|tsx)$',
            r'index\.(js|ts|jsx|tsx)$',
            r'main\.(js|ts|jsx|tsx)$',
        ]
        
        # Check if file matches inclusion patterns
        matches_inclusion = any(re.search(pattern, file_path, re.IGNORECASE) for pattern in include_patterns)
        
        # If it matches inclusion patterns, check if it's an Express.js file
        if matches_inclusion:
            return self._is_express_file(content)
            
        # For other files, require stronger Express.js indicators
        return self._is_express_file(content) and self._has_route_definitions(content)
    
    def _has_route_definitions(self, content: str) -> bool:
        """Check if content contains actual route definitions"""
        route_definition_patterns = [
            # Standard Express route patterns
            r'(router|app)\.(get|post|put|delete|patch|head|options|all)\s*\(',
            r'(router|app)\.use\s*\(',
        ...
        return any(re.search(pattern, content, re.IGNORECASE) for pattern in route_definition_patterns)
    ...
    def _filter_comments_and_documentation(self, content: str) -> str:
        """
        CRITICAL FIX: Filter out comments and documentation to prevent false positives
        
        This prevents detection of route patterns in:
        - Single-line comments: // router.get('${route path}', ...)
        - Multi-line comments: /* router.get(...) */
        - JSDoc comments: /** @example router.get(...) */
        - String literals containing route patterns
        """
        # Remove single-line comments (but preserve line structure for line numbers)
        content = re.sub(r'//.*$', '', content, flags=re.MULTILINE)
        
        # Remove multi-line comments including JSDoc
        content = re.sub(r'/\*[\s\S]*?\*/', '', content)
        
        # Remove string literals that might contain route patterns in documentation
        # This is more conservative - only remove strings that look like documentation
        content = re.sub(r'["\'].*?router\.(get|post|put|delete|patch|head|options|all).*?["\']', '""', content, flags=re.IGNORECASE)
        
        # Remove template literals that contain route examples
        content = re.sub(r'`.*?router\.(get|post|put|delete|patch|head|options|all).*?`', '``', content, flags=re.IGNORECASE)
