...
from detectors.flutter_detector import FlutterDetector
...
class AttackSurfaceScanner:
    ...
    def _initialize_detectors(self) -> List:
        ...
        # Mobile frameworks
        if Framework.FLUTTER in framework_values:
            detectors.append(FlutterDetector())
        
        self.logger.info(f"Initialized {len(detectors)} detectors for frameworks: {[d.__class__.__name__.replace('Detector', '') for d in detectors]}")
        return detectors
    ...
    async def scan_repository(self, quiet: bool = False) -> ScanResult:
        """
        ...
        if not quiet:
            console.print("[bold blue]🔍 Discovering files (optimized)...")
        ...
        if not quiet:
            console.print(f"[bold blue]🔍 Analyzing routes ({self.repository_metrics.recommended_strategy.value})...")
        services = await self._scan_files_adaptive(files_to_scan)
        
        # Perform AI analysis if enabled
        if self.ai_analyzer and not quiet:
            console.print("[bold blue]🤖 Running AI analysis...")
            services = self._enhance_with_ai_analysis(services)
        
        # Calculate risk scores
        if not quiet:
            console.print("[bold blue]⚡ Calculating risk scores...")
        services = self._calculate_risk_scores(services)
        
        # Generate scan result
        if not quiet:
            console.print("[bold blue]📊 Generating results...")
        ...
        self._log_scan_summary(scan_result, quiet)
        
        # Phase 4: Display performance report (only if enabled and not quiet)
        if self.config.performance_report and not quiet:
            performance_report = self.performance_optimizer.get_performance_report()
            self._display_performance_report(performance_report, quiet)
    ...
    async def _scan_sequential_with_cache(self, files: List[str]) -> Dict[str, ServiceInfo]:
        ...
        with Progress(
            ...
            task = progress.add_task(
                f"[cyan]Processing {len(files):,} files sequentially", 
                total=len(files)
            )
    ...
    async def _scan_balanced_parallel(self, files: List[str], max_workers: int) -> Dict[str, ServiceInfo]:
        ...
        with Progress(
            ...
            # Create the main task
            task = progress.add_task(
                f"[cyan]Processing {len(files):,} files with {max_workers} workers", 
                total=len(files)
            )
    ...
    async def _scan_chunked_with_cache(self, files: List[str], max_workers: int, 
        ...
        with Progress(
            ...
            # Create the main task
            task = progress.add_task(
                f"[cyan]Processing {len(files):,} files in {total_chunks} chunks", 
                total=total_chunks
            )
            ...
            for chunk_start in range(0, len(files), chunk_size):
                ...
                progress.update(task, description=f"[cyan]Processing chunk {current_chunk}/{total_chunks} (files {chunk_start:,}-{chunk_end:,})")
    ...
    def _add_routes_to_services(self, services: Dict[str, ServiceInfo], file_path: str, 
        ...
        if file_routes:
            ...
            # Add routes with global deterministic deduplication and commit info
            for route in file_routes:
                ...
                if route_key not in self.global_seen_routes:
                    ...
                    # Add commit information to the route if enabled
    ...
    async def scan_prefixes_only(self):
        ...
        console.print(Panel(
            "[bold blue]🔍 EndPointHawk Prefix Discovery Mode[/bold blue]\n"
    ...
    def _display_terminal_output(self, scan_result):
        """Display scan results in a modern, clean terminal format"""
        from rich.table import Table
        from rich.panel import Panel
        
        # Modern header
        console.print(Panel(
            f"[bold blue]🔍 Detected Routes[/bold blue]\n"
            f"[dim]Showing up to 50 routes for terminal display[/dim]",
            border_style="blue",
            padding=(0, 1)
        ))
        ...
        table.add_column("Method", style="cyan", width=8, justify="center")
        table.add_column("Route", style="white", width=50)
        table.add_column("Framework", style="green", width=12)
        table.add_column("Risk", style="red", width=8, justify="center")
        
        # Add routes to table
        route_count = 0
        max_routes = 50  # Reduced for cleaner display
        ...
        for service_name, service in services_iter:
            for route in service.routes:
                ...
                framework_str = str(route.framework)
        ...
        if scan_result.total_routes > max_routes:
            console.print(Panel(
                f"[yellow]📝 Showing first {max_routes} routes[/yellow]\n"
                f"[dim]Total routes: {scan_result.total_routes}[/dim]\n"
                f"[dim]Use --output-format csv or json to export all routes[/dim]",
                border_style="yellow",
                padding=(0, 1)
            ))
    
    def _log_scan_summary(self, scan_result: ScanResult, quiet: bool = False):
        """Log scan summary to console with modern, clean design."""
        from rich.panel import Panel
        from rich.columns import Columns
        from rich.text import Text
        
        # Create modern summary display
        if not quiet:
            console.print("\n")
        
        # Risk summary with colored indicators
        risk_summary = Columns([
            Panel(
                f"[bold white]{scan_result.total_routes}[/bold white]\n[dim]Total Routes[/dim]",
                border_style="blue",
                padding=(0, 1)
            ),
            Panel(
                f"[bold red]{scan_result.high_risk_routes}[/bold red]\n[dim]High Risk[/dim]",
                border_style="red",
                padding=(0, 1)
            ),
            Panel(
                f"[bold yellow]{scan_result.medium_risk_routes}[/bold yellow]\n[dim]Medium Risk[/dim]",
                border_style="yellow",
                padding=(0, 1)
            ),
            Panel(
                f"[bold green]{scan_result.low_risk_routes}[/bold green]\n[dim]Low Risk[/dim]",
                border_style="green",
                padding=(0, 1)
            ),
            Panel(
                f"[bold cyan]{len(scan_result.services)}[/bold cyan]\n[dim]Services[/dim]",
                border_style="cyan",
                padding=(0, 1)
            )
        ], expand=True)
        
        console.print(risk_summary)
        
        # Framework distribution
        if scan_result.routes:
            frameworks = {}
            ...
            if frameworks:
                framework_text = "  ".join([f"[bold]{f}[/bold]: {c}" for f, c in frameworks.items()])
                console.print(Panel(
                    f"[dim]Framework Distribution:[/dim]\n{framework_text}",
                    border_style="dim",
                    padding=(0, 1)
                ))
        ...
        if scan_result.high_risk_routes > 0:
            console.print(Panel(
                f"[bold red]⚠️  {scan_result.high_risk_routes} high-risk routes detected![/bold red]\n"
                f"[dim]Review the generated reports for security recommendations.[/dim]",
                border_style="red",
                padding=(0, 1)
            ))
    ...
    def export_results(self, scan_result: ScanResult):
        ...
        for format_type, exporter in self.exporters.items():
            try:
                output_file = output_dir / f"endpointhawk_report_{repo_name}.{format_type}"
    ...
    def _display_performance_report(self, performance_report: Dict[str, Any], quiet: bool = False):
        """Display performance optimization report with modern design"""
        from rich.panel import Panel
...
def auto_detect_frameworks(repo_path: str) -> List[Framework]:
    ...
    # Flutter - look for pubspec.yaml and dart files
    pubspec_files = list(repo_path.rglob("pubspec.yaml"))
    dart_files = list(repo_path.rglob("*.dart"))
    if pubspec_files or len(dart_files) > 10:  # If there are many dart files, it's likely Flutter
        frameworks.add(Framework.FLUTTER)
...
async def _run_async_scan(config: ScanConfig, prefixes_only: bool, verbose: bool, output_format: str, quiet: bool = False):
    ...
    # Full scan mode
    scanner = AttackSurfaceScanner(config)
    scan_result = await scanner.scan_repository(quiet)
...
@click.option('--frameworks', default='auto', help='Frameworks to scan (nestjs,express,go,python,grpc,flutter,all,auto)')
...
@click.option('--include-commit-info/--no-commit-info', default=True, 
              help='Include git commit information (author, date, hash) for each route')
...
@click.option('--quiet', is_flag=True, help='Minimal output mode (summary only)')
...
def main(repo_path, output_dir, frameworks, risk_threshold, 
         use_ai, organization_patterns, resolve_prefixes, include_commit_info, prefix_config, prefixes_only, verbose,
         ...
         performance_mode, cache_enabled, cache_cleanup, max_memory, performance_report, chunk_size, max_workers, progress_mode, quiet,
    ...
    try:
        ...
        # Configuration loaded silently - no need to display this to users
    ...
    # --- MOVE FRAMEWORK DETECTION TO THE TOP ---
    # Early framework detection for accurate display and initialization
    original_frameworks = frameworks  # Store original value for display logic
    detected_frameworks = []
    if not frameworks or frameworks == 'auto':
        detected_frameworks = auto_detect_frameworks(repo_path)
        if detected_frameworks:
            # Don't modify frameworks variable, just use detected_frameworks directly
            pass
        else:
            # Fallback to all supported frameworks if none detected
            detected_frameworks = [Framework.NESTJS, Framework.EXPRESS, Framework.NEXTJS, Framework.GO_HTTP, Framework.FASTAPI, Framework.DJANGO, Framework.FLASK, Framework.GRPC, Framework.FLUTTER]
    else:
        # Parse frameworks from CLI/config
        if frameworks == 'all':
            # Only include actual frameworks, not UNKNOWN
            detected_frameworks = [f for f in Framework if f != Framework.UNKNOWN]
        else:
            framework_map = {
                'nestjs': Framework.NESTJS,
                'express': Framework.EXPRESS,
            ...
            detected_frameworks = [framework_map.get(f.strip().lower(), f) for f in frameworks.split(',') if f.strip()]
    ...
    # Display frameworks info - show detected frameworks or auto-detecting status
    if original_frameworks == 'auto':
        # Show detected frameworks if auto-detection found any, otherwise show "Auto-detecting..."
        framework_names = []
        for f in config.frameworks:
            if hasattr(f, 'value'):
                framework_name = f.value
            else:
                framework_name = str(f)
            # Only include actual frameworks, not 'Unknown'
            if framework_name.lower() != 'unknown':
                framework_names.append(framework_name.title())
        
        if framework_names:
            frameworks_display = ', '.join(framework_names)
        else:
            frameworks_display = "Auto-detecting..."
    elif original_frameworks == 'all':
        frameworks_display = "All Frameworks"
    else:
        # Show detected frameworks - filter out 'Unknown' and show only actual frameworks
        framework_names = []
        for f in config.frameworks:
            if hasattr(f, 'value'):
                framework_name = f.value
    ...
    # Modern, clean display with professional branding (skip if quiet mode)
    if not quiet:
        console.print("\n")
        
        # Professional EndPointHawk logo - Compact for smaller terminals
        console.print(Panel(
            "[bold blue]"
            "                            /^^^\\                            \n"
            "                           / o o \\                           \n"
            ...
            "                         /_________\\                         \n"
            "[/bold blue]\n"
            "[bold cyan]EndPointHawk[/bold cyan]\n"
            "[dim]Developed by Ranjan Kumar (@rootranjan)[/dim]",
            border_style="blue",
            padding=(1, 2)
        ))
        ...
        console.print(Panel(
            ...
            f"[cyan]Commit Info:[/cyan] {'✅ Enabled' if include_commit_info else '❌ Disabled'}",
    ...
    # Framework detection already completed above for accurate display
    
    # Run async operations
    try:
        asyncio.run(_run_async_scan(config, prefixes_only, verbose, primary_format, quiet))
