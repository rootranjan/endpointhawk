#!/usr/bin/env python3
"""
Enterprise Configuration Management for EndPointHawk
...
class ConfigurationManager:
    ...
    def _get_default_config_paths(self) -> List[str]:
        ...
        system_config = "/etc/endpointhawk/config.yaml"
        if os.path.exists(system_config):
            paths.append(system_config)
        
        # User configuration
        home_config = Path.home() / ".endpointhawk" / "config.yaml"
        if home_config.exists():
            paths.append(str(home_config))
        
        # Project-local configuration
        for config_name in ["endpointhawk.yaml", "endpointhawk.yml", ".endpointhawk.yaml"]:
            if os.path.exists(config_name):
                paths.append(config_name)
        
        # Environment variable override
        env_config = os.getenv("ENDPOINTHAWK_CONFIG")
    ...
    def _load_environment_overrides(self):
        ...
        if os.getenv('ENDPOINTHAWK_AI_ENABLED'):
            self.config.security.enable_ai_analysis = os.getenv('ENDPOINTHAWK_AI_ENABLED').lower() == 'true'
        
        if os.getenv('ENDPOINTHAWK_RISK_THRESHOLD'):
            self.config.security.default_risk_threshold = os.getenv('ENDPOINTHAWK_RISK_THRESHOLD')
        
        # Performance settings from environment
        if os.getenv('ENDPOINTHAWK_MAX_WORKERS'):
            try:
                self.config.performance.max_workers = int(os.getenv('ENDPOINTHAWK_MAX_WORKERS'))
            except ValueError:
                pass
        
        if os.getenv('ENDPOINTHAWK_MEMORY_LIMIT'):
            try:
                self.config.performance.memory_limit_mb = int(os.getenv('ENDPOINTHAWK_MEMORY_LIMIT'))
            except ValueError:
                pass
        
        # Organization settings from environment
        if os.getenv('ENDPOINTHAWK_ORG_NAME'):
            self.config.organization.organization_name = os.getenv('ENDPOINTHAWK_ORG_NAME')
